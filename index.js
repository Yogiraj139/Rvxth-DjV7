...
console.log('Build: Rvxth DJ V22 bgutil-provider-support');
...
function normalizeExtractorEnvValue(rawValue, assignmentNames) {
  ...
  const escapedNames = assignmentNames.map((name) => name.replace(/[.*+?^${}()|[\]\\]/g, '\\$&'));
  const assignmentMatch = value.match(new RegExp(`^(?:${escapedNames.join('|')})\\s*=\\s*(.+)$`, 'i'));
...
function getYoutubePoTokenConfig() {
  const poTokenB64Env = getEnvValue(['YOUTUBE_PO_TOKEN_B64', 'YT_PO_TOKEN_B64']);
  const poTokenEnv = poTokenB64Env.value
    ? poTokenB64Env
    : getEnvValue(['YOUTUBE_PO_TOKEN', 'YT_PO_TOKEN']);
  const rawToken = poTokenB64Env.value
    ? decodeBase64Value(poTokenB64Env.value, poTokenB64Env.name)
    : poTokenEnv.value;
  const token = normalizeExtractorEnvValue(rawToken, [
    'YOUTUBE_PO_TOKEN',
    'YT_PO_TOKEN',
    'YOUTUBE_PO_TOKEN_B64',
    'YT_PO_TOKEN_B64'
  ]);

  if (!token) {
    return { source: poTokenEnv.name, value: '' };
  }

  return {
    source: poTokenEnv.name,
    value: token.includes('+') ? token : `mweb.gvs+${token}`
  };
}

const youtubePoToken = getYoutubePoTokenConfig();
const youtubePotProviderUrl = normalizeExtractorEnvValue(
  getEnvValue(['YOUTUBE_POT_PROVIDER_URL', 'BGUTIL_POT_PROVIDER_URL']).value,
  ['YOUTUBE_POT_PROVIDER_URL', 'BGUTIL_POT_PROVIDER_URL']
);
const youtubePlayerClients = (
  process.env.YOUTUBE_PLAYER_CLIENTS?.trim() ||
  (youtubePoToken.value || youtubePotProviderUrl
    ? 'mweb,web_safari,tv,tv_embedded,web_embedded,tv_simply,android_vr,android_sdkless,android,ios,web'
    : 'web_safari,tv,tv_embedded,web_embedded,mweb,android,ios,web,tv_simply,android_sdkless')
)
  .split(',')
  .map((clientName) => clientName.trim())
  .filter(Boolean);
const youtubeExtractorFormats =
  process.env.YOUTUBE_EXTRACTOR_FORMATS?.trim() || 'missing_pot,incomplete';

console.log(`yt-dlp path: ${ytDlpPath}`);
console.log('yt-dlp playback mode: direct format URL picker');
console.log(`YouTube player clients: ${youtubePlayerClients.join(', ')}`);
console.log(`YouTube extractor formats: ${youtubeExtractorFormats}`);
console.log(
  `YouTube PO token: ${youtubePoToken.value ? `configured from ${youtubePoToken.source}` : 'not configured'}`
);
console.log(
  `YouTube POT provider: ${youtubePotProviderUrl || 'not configured'}`
);
...
function youtubeClientArgs(clientName) {
  const extractorParts = [];
  const args = [];

  if (clientName) {
    extractorParts.push(`player_client=${clientName}`);
  }

  if (youtubeExtractorFormats) {
    extractorParts.push(`formats=${youtubeExtractorFormats}`);
  }

  if (youtubePoToken.value) {
    extractorParts.push(`po_token=${youtubePoToken.value}`);
  }

  if (extractorParts.length) {
    args.push('--extractor-args', `youtube:${extractorParts.join(';')}`);
  }

  if (youtubePotProviderUrl) {
    args.push('--extractor-args', `youtubepot-bgutilhttp:base_url=${youtubePotProviderUrl}`);
  }

  return args;
}

function runYtDlpJson(target, clientName = null) {
  return new Promise((resolve, reject) => {
    const process = spawn(
      ytDlpPath,
      ytDlpArgs(
        target,
        ...youtubeClientArgs(clientName),
        '--dump-single-json',
        '--skip-download',
...
function runYtDlpGetUrl(target, format, clientName = null) {
  return new Promise((resolve, reject) => {
    const args = [
      target,
      ...youtubeClientArgs(clientName),
...
function scoreFormat(format) {
  ...
  if (String(format.protocol ?? '').includes('m3u8')) {
    score += 120;
  }

  if (format.ext === 'webm' || format.ext === 'm4a' || format.ext === 'mp4' || format.ext === 'm3u8') {
    score += 50;
  }
...
async function resolvePlaybackSource(song) {
  ...
  for (const clientName of youtubePlayerClients) {
    try {
      console.log(`Trying metadata formats with YouTube client: ${clientName}`);
      const info = await runYtDlpJson(song.url, clientName);
      const video = info.entries?.[0] ?? info;
      lastVideo = video;
      ...
      if (selected) {
        console.log(
          `Selected YouTube format: ${selected.id} ext=${selected.ext} acodec=${selected.acodec} vcodec=${selected.vcodec} client=${clientName}`
        );
      ...
      errors.push(`${clientName}: metadata had ${count} formats but none had audio URLs`);
    } catch (error) {
      const detail = `${clientName}: metadata failed: ${friendlyError(error)}`;
      errors.push(detail);
      console.warn(detail);
    }
  }
  ...
  const fallbackFormats = [
    'best',
    '18/best[height<=360][acodec!=none]',
    'best[acodec!=none]',
    'bestaudio*/best',
    'worst',
    null
  ];
  const clientsForUrl = [...youtubePlayerClients, null];

  for (const clientName of clientsForUrl) {
    for (const format of fallbackFormats) {
      try {
        console.log(
          `No usable metadata formats. Trying yt-dlp --get-url fallback: client=${clientName ?? 'default'} format=${format ?? 'default'}`
        );
        const url = await runYtDlpGetUrl(song.url, format, clientName);

        return {
          id: `get-url:${clientName ?? 'default'}:${format ?? 'default'}`,
          ext: 'unknown',
          acodec: 'unknown',
          vcodec: 'unknown',
          url,
          headers: buildHeaderString(lastVideo?.http_headers)
        };
      } catch (error) {
        const detail = `${clientName ?? 'default'}/${format ?? 'default'}: ${friendlyError(error)}`;
        errors.push(detail);
        console.warn(`yt-dlp --get-url fallback failed (${detail})`);
      }
    }
  }

  const shortErrors = errors.slice(-5).join(' | ');
  throw new Error(
    `YouTube did not return any playable audio formats for this video. Last tries: ${shortErrors}`
  );
}
