...
const pluginsDir = path.join(binDir, 'yt-dlp-plugins');
const bgutilPluginPath = path.join(pluginsDir, 'bgutil-ytdlp-pot-provider.zip');
const downloadUrl =
  process.env.YTDLP_DOWNLOAD_URL ||
  `https://github.com/yt-dlp/yt-dlp/releases/latest/download/${downloadName}`;
const bgutilPluginUrl =
  process.env.BGUTIL_POT_PLUGIN_URL ||
...
async function downloadFile(url, destination, options = {}) {
  ...
  if (validate && looksBroken(tempPath)) {
    fs.rmSync(tempPath, { force: true });
    throw new Error('Downloaded yt-dlp file looks invalid.');
  }

  if (!validate && fs.statSync(tempPath).size < 1_000) {
    fs.rmSync(tempPath, { force: true });
    throw new Error('Downloaded plugin file looks too small.');
  }
...
if (process.env.INSTALL_BGUTIL_POT_PROVIDER !== '0') {
  console.log(`Preparing bgutil POT provider plugin from ${bgutilPluginUrl}`);
  await downloadFile(bgutilPluginUrl, bgutilPluginPath, { validate: false });
  console.log(`bgutil POT provider plugin ready at ${bgutilPluginPath}`);
}
